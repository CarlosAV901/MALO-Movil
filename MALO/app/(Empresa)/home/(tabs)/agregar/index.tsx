...
export default function AgregarEmpleo  ()  {
 ...
 // console.log("empresa", user);
  ...
  const handleSubmit = async () => {
    ...
    const multimediaTipo = 'image/jpeg'; // Ajusta según el tipo que estás usando
  
    // Crea una instancia de URLSearchParams y agrega cada parámetro
    const urlParams = new URLSearchParams();
    urlParams.append("titulo", titulo);
    urlParams.append("descripcion", descripcion);
    urlParams.append("empresa_id", user.id);
    urlParams.append("ubicacion", ubicacion);
    urlParams.append("salario_minimo", salarioMinimo.toString());
    urlParams.append("salario_maximo", salarioMaximo.toString());
    urlParams.append("horario", horario);
    urlParams.append("multimediaNombre", multimediaNombre || "imagen");
    urlParams.append("multimediaTipo", multimediaTipo);
    urlParams.append("multimediaContenido", multimediaContenido);
    urlParams.append("ContentType", "application/json");
    urlParams.append("ContentDisposition", "inline");
    urlParams.append("Length", "0"); 
    urlParams.append("Name", multimediaNombre || "imagen");
    urlParams.append("FileName", multimediaNombre || "archivo.jpg");
  
    try {
      const response = await fetch(`https://malo-backend-empleos.onrender.com/api/Empleo/PostEmpleo?${urlParams.toString()}`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        console.log("Respuesta de error del servidor:", response.statusText);
